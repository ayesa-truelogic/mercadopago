...
class TestMercadoPago < MiniTest::Unit::TestCase
  ...
  #
  CREDENTIALS = { :client_id => '4728653529051976', :client_secret => 'FBWIoUID2acjEb53sq05MaVgcs2t4s87' }

  #
  # Example payment request.
  #
  PAYMENT_REQUEST = {
    :external_reference => 'TEST-OP-ID-1234',
    :items => [
      {
        :id =>          'Codigo 123',
  ...
  def test_that_client_can_create_payment_preference_in_sandbox
    ...
    response = mp_client.create_preference(PAYMENT_REQUEST)
    assert !response['sandbox_init_point'].nil?
  end
  ...
  def test_that_client_can_get_preference_in_sandbox
    ...
    response = mp_client.get_preference(pref_id)
    assert_equal "https://sandbox.mercadopago.com/mla/checkout/pay?pref_id=#{pref_id}", response['sandbox_init_point']
  end
